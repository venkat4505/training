...
int myOpen(struct inode *inode, struct file *filep)
{
	...
	printk(KERN_CONT "Open successful\n");
	return 0;
}
...
int myRelease(struct inode *in, struct file *fp)
{
	printk(KERN_CONT "File released\n");
	return 0;
}
...
static int __init myInit(void)
{
	...
	printk(KERN_CONT "Initializing character device\n");
	...
	printk(KERN_CONT "Major number allocated = %d\n", MAJOR(mydev));
...
static void myExit(void)
{
	printk(KERN_CONT "Exiting the character driver \n");
...
long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	...
	printk(KERN_CONT "LWP\tPLWP\tPID\tPPID\n");

	list_for_each_entry(task, &(ptr -> tasks),tasks) {

	printk(KERN_CONT " %d\t",task -> pid);
	printk(KERN_CONT " %d\t",task -> parent -> pid);
	printk(KERN_CONT " %d\t",task -> tgid);  
	printk(KERN_CONT " %d\n",task -> parent -> tgid);
