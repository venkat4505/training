...
long myIoctl(struct file* fp1,unsigned int pid,unsigned long b)
{
	...
	struct vm_area_struct *vm_new = NULL;//vm_new holds address of vm_area_struct

	ptr = find_task_by_vpid(pid);//ptr holds address of task_struct
	vm_new = ptr -> mm -> mmap;//traversing to vm_area_struct

	printk(KERN_CONT "find_task_by_vpid = %d\n",ptr->pid);//accressing member pid in structure task_struct
	...
	while (vm_new != NULL) {
		...
		printk(KERN_CONT"%08x\t",vm_new->vm_pgoff);//accsessing member offset in vm_area_struct

		((vm_new -> vm_file) == NULL) ? printk(KERN_CONT"%-7d\n",0) : 
				printk(KERN_CONT"%-7d\n",vm_new -> vm_file -> f_inode -> i_ino);//inode number

		vm_new = vm_new -> vm_next;//next list ie..code to data 
	} 
	return 0;
}
